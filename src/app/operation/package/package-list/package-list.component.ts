...
export class PackageListComponent extends PackageDataComponent implements OnInit {
    ...
    search(type = 'search', id = null) {
        if (id !== this.manifest_id) {
            ...
            this.packageService.searchUsSending(params).subscribe(response => {
    ...
    merge() {
        ...
        this.packageService.popup.confirm(() => {
            this.packageService.post('s-us-send', this.trackingMerge).subscribe(rs => {
                const res: any = rs;
                if (res.success) {
    ...
    updateSellerRefund() {
        ...
        this.packageService.sellerRefundUsSending(this.sellerRefundForm.id, this.sellerRefundForm).subscribe(rs => {
            const res: any = rs;
            if (res.success) {
                this.popUp.success(res.message);
                this.search('tracking');
    ...
    update() {
        ...
        this.packageService.put('us-sending/' + this.updateForm.id, this.updateForm).subscribe(rs => {
            const res: any = rs;
            if (res.success) {
                this.popUp.success(res.message);
